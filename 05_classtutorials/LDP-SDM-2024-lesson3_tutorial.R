...
# - # - #   Lesson 3: Introduction to tools for data validation     # - # - #
# - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - #

# - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - #
# - # - #   SELF-DIRECTED TUTORIAL -- work through before class     # - # - #
# - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - # - #
...
## plot the data (takes a minute to construct this)
penguins %>%
...
species.list <- c("adeliae", "antarcticus", "papua")

i <- adist(penguins$speciesL, species.list) %>% 
  apply(1, which.min)

(species.fixed <- tibble(speciesL_original = penguins$speciesL, 
...
penguins <- bind_cols(penguins, species.fixed[2])
...
## extract upstream taxonomic information for the species in the penguins
## dataset, and create a new table with this information

## create a list of unique binomials
species.list <- penguins %>% 
  distinct(species = speciesL_repaired) %>% 
  ## add a column with the genus
  mutate(genus = "Pygoscelis", .before = 1) %>% 
  ## join genus and species into a binomial
  unite(binomial, c("genus", "species"), sep = " ")

## search for upstream taxonomy
(penguins.tax <- classification(unlist(species.list), db = "itis"))

## convert this into a useable data format (from a nested list to a tibble)
penguins.tax <- penguins.tax %>% 
...
## we *could* also add this into the penguins dataset
## (but note that this results in a lot of repeated taxonomic information 
## -- remember tidy data and relational design principles!)
penguins %>% 
  mutate(genus = "Pygoscelis", .before = 1) %>% 
  unite(binomial, c("genus", "speciesL_repaired"), sep = " ") %>% 
  left_join(., penguins.tax, by = c("binomial" = "species")) %>% 
  View()
